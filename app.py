...
@app.route('/data/datasetList_auto.json')
@authRequired
def autogenerateDataSpecList() -> str:
    ...
    responseFields = "nextPageToken, files(id, parents, size, modifiedTime)"
    ...
    for index, candidateFile in enumerate(candidateFiles):
        ...
        if dataSpecObj == None:
            skipCount += 1
            continue
        filename = dataSpecObj['folder'].replace('/', '__')
        filenameSet.add(filename)
        newFile = open('./static/cache/datasetList/initGen/' + filename + '.json', 'w')
        newFile.write(json.dumps(dataSpecObj))
    print('')
    print('done')
...
def getDataSpecObj(service, googleDriveFile) -> Union[Dict, None]:
    dataSpecObj = {}
    driveId = googleDriveFile['id']
    folderPathList = getFolderPathFromGoogleDrive(service, TOP_GOOGLE_DRIVE_FOLDER_ID, googleDriveFile['parents'][0])
    if len(folderPathList) == 0:
        return None
    dataSpecObj['googleDriveId'] = driveId
