...
export class Plot2dPathsWidget extends BaseWidget<CurveList, DatasetSpec> {
    ...
    protected initProps(props?: any[]): void {
        ...
        let initialOption =
            this.quickPickOptions[this.initialQuickPickOptionIndex];
    ...
    protected init(): void {
        ...
        this._titleBarContainer = containerSelect
            .append('div')
            .classed('titleBarContainer', true);

        this._svgSelect = containerSelect.append('svg');
        this._mainGroupSelect = this.svgSelect
            .append('g')
            .attr(
                'transform',
        ...
        this._canvasElement = this.canvasContainer
            .append('xhtml:canvas')
            .attr('width', this.vizWidth)
            .attr('height', this.vizHeight)
        ...
        if (this.canBrush) {
            this._brushGroupSelect = this.svgSelect
                .append('g')
                .attr(
                    'transform',
                    `translate(${this.margin.left}, ${this.margin.top})`
        ...
        this._xAxisGroupSelect = this.svgSelect
            .append('g')
            .attr(
                'transform',
                `translate(${this.margin.left}, ${
                    this.margin.top + this.vizHeight
                })`
            )
            .classed('labelColor', true);

        this._yAxisGroupSelect = this.svgSelect
            .append('g')
            .attr(
                'transform',
                `translate(${this.margin.left}, ${this.margin.top})`
        ...
        this._currentFrameIndicator = this.svgSelect
            .append('g')
            .attr(
                'transform',
                `translate(${this.margin.left}, ${this.margin.top})`
        ...
        this._averageCurveLabelContainer = this.svgSelect
            .append('g')
            .attr(
                'transform',
                `translate(${this.margin.left}, ${this.margin.top})`
        ...
        this._averageLegendSelect = this.svgSelect
            .append('g')
            .attr(
                'transform',
                `translate(${this.margin.left / 2}, ${
                    this.margin.top + this.vizHeight + 44
                })`
            );

        this._facetLegendSelect = this.svgFacetSelect
            .append('g')
            .attr(
                'transform',
                `translate(${this.margin.left / 2}, ${
        ...
        this._selectConditionButton = DevlibTSUtil.getIconButton(
            'th',
            () => {
                this._inFacetMode = true;
        ...
        this.selectConditionButton.title =
            'Filter data on experimental conditions';
        ...
        this._compareConditionButton = DevlibTSUtil.getIconButton(
            'chart-line',
            () => {
                this._inFacetMode = false;
        ...
        this.compareConditionButton.title =
            'Compare average curves across experimental conditions';
    ...
    private initQuickPickOptions(): void {
        ...
        this._quickPickContainerSelect = this.titleBarContainer
            .append('div')
        ...
        for (let quickPickOption of this.quickPickOptions) {
            ...
            if (quickPickOption.averaged) {
                optionName =
                    'Averaged: ' +
                    quickPickOption.yKey +
                    ' over ' +
                    quickPickOption.xKey;
            } else {
                optionName =
                    quickPickOption.yKey + ' v. ' + quickPickOption.xKey;
            }
            let buttonProp: ButtonProps = {
                ...
                callback: () =>
                    this.changeAxes(
                        quickPickOption.xKey,
                        quickPickOption.yKey,
                        quickPickOption.averaged,
                        quickPickOption.squareAspectRatio
                    )
            };
            buttonPropList.push(buttonProp);
        }
        this.quickPickOptionSelect.onDataChange(
            buttonPropList,
            this.initialQuickPickOptionIndex
        );
    }
    ...
    private addLabel(): void {
        this._xLabelTextSelect = this.svgSelect
            .append('text')
            .classed('axisLabel', true)
            .classed('labelColor', true)
            .classed('noDisp', true);

        this._yLabelTextSelect = this.svgSelect
            .append('text')
            .classed('axisLabel', true)
            .classed('labelColor', true)
            .classed('noDisp', true);
    ...
    private positionLabels(): void {
        ...
        this.xLabelTextSelect.attr(
            'transform',
            `translate(${this.margin.left + this.vizWidth / 2}, ${
                this.margin.top + this.vizHeight + bufferForAxis
            })`
        );
    ...
    private changeAxes(
        ...
        document.dispatchEvent(
            new CustomEvent('averageCurveKeyChange', {
                detail: {
                    yKey: yKey
                }
            })
    ...
    private updateScales(): void {
        ...
        if (this.inAverageMode) {
            minY = d3.min(this.facetList, facet =>
                d3.min(
                    (facet.data as CurveList).getAverageCurve(
                        this.yKey,
                        false,
                        this.smoothCurves
                    ),
            ...
            maxY = d3.max(this.facetList, facet =>
                d3.max(
                    (facet.data as CurveList).getAverageCurve(
                        this.yKey,
                        false,
    ...
    private makeSquareAspectRatioScales(
        minX: number,
        maxX: number,
        minY: number,
        maxY: number
    ...
    private makeStretchedAspectRatioScales(
        minX: number,
        maxX: number,
        minY: number,
        maxY: number
    ...
    private swapSvgVisibility(): void {
        ...
        if (this.inFacetMode && this.dataSuperset) {
            ...
            let applyButton = document.getElementById(
                'conditionFilterApplyButton'
            );
            if (applyButton) {
    ...
    private updateAllPaths(): void {
        let line = d3
            .line<PointND>()
            .x((d, i) => {
                return this.scaleX(d.get(this.xKey));
            })
            .y(d => {
                return this.scaleY(d.get(this.yKey));
            })
            .defined(d => d.inBrush);
    ...
    private updateAveragePaths(): void {
        ...
        for (let i = this.facetList.length - 1; i >= 0; i--) {
            ...
            canvasContext.strokeStyle = GroupByWidget.getColor(
                facet.name,
                this.colorLookup
            );
            let dataPoints = facet.data.getAverageCurve(
                this.yKey,
                false,
                this.smoothCurves
            );
            ...
            if (this.data.brushApplied) {
                let filteredDataPoints = facet.data.getAverageCurve(
                    this.yKey,
                    true,
                    this.smoothCurves
                );
    ...
    private updateFacetPaths(): void {
        ...
        this.mainGroupFacetSelect.attr(
            'transform',
            `translate(${margin.left}, ${margin.top})`
        );

        let rowSelect = this.mainGroupFacetSelect
            .selectAll('g.row')
            ...
            .attr(
                'transform',
                (_, i) => `translate(0, ${i * (miniSize + miniPadding)})`
            );

        this._miniCellSelect = rowSelect
            .selectAll('g.miniCell')
            ...
            .attr(
                'transform',
                (_, i) => `translate(${i * (miniSize + miniPadding)}, 0)`
            ) as SvgSelection;
        ...
        let [minMassFull, maxMassFull] = this.getExtentOfGrowthCurves(
            defaultFacetsFull,
            true
        );
        ...
        this.miniCellSelect
            .selectAll('.miniExemplarArea')
            .data(d => [d])
            .join('path')
            .classed('miniExemplarArea', true)
            .attr('d', d => {
                let pathString = this.getGrowthLine(
                    d,
                    defaultFacets,
                    lineAvg,
                    false,
        ...
        this.miniCellSelect
            .selectAll('.miniExemplarCurve.allData')
            .data(d => [d])
            .join('path')
        ...
        if (this.data.brushApplied) {
            this.miniCellSelect
                .selectAll('.miniExemplarCurve.selection')
                .data(d => [d])
                .join('path')
                .classed('miniExemplarCurve', true)
                ...
                .classed('active', true)
                .attr('stroke', d =>
                    GroupByWidget.getColor(d, this.colorLookup)
                )
                .attr('d', d => {
                    return this.getGrowthLine(
                        d,
        ...
        this.yAxisFacetSelect
            ...
            .attr(
                'transform',
                `translate(${-maxLabelWidth - labelPadding - mainLabelSize}, ${
                    vizHeight / 2
                }) rotate(-90)`
            )
        ...
        this.yAxisFacetSelect
            ...
            .attr(
                'transform',
                (d, i) =>
                    `translate(${-maxLabelWidth - labelPadding}, ${
                        i * (miniSize + miniPadding)
                    })`
            )
            .selectAll('div')
            .data(d => [d])
            ...
            .attr(
                'style',
                `max-width: ${maxLabelWidth}px; min-width: ${maxLabelWidth}px;`
            )
            .attr('title', d => d)
            .on('click', d => {
                if (this.allConditionsTrue()) {
                ...
                let newValue: boolean = !Array(...rowMap.values()).every(
                    x => x
                );
            ...
            .on('mouseleave', () =>
                this.miniCellSelect.selectAll('rect').classed('hovered', false)
            )
            .text(d => d);

        const maxLabelHeight = 36;

        this.xAxisFacetSelect.attr(
            'transform',
            `translate(${margin.left}, ${margin.top + vizHeight})`
        );
        ...
        this.xAxisFacetSelect
            ...
            .attr(
                'transform',
                `translate(${vizWidth / 2}, ${
                    maxLabelHeight + 2 * labelPadding
                })`
            )
            .classed('mediumText', true)
            .text(d => d);
        ...
        this.xAxisFacetSelect
            ...
            .attr(
                'style',
                `max-width: ${miniSize}px; min-width: ${miniSize}px; height: ${maxLabelHeight}px`
            )
            .attr('title', d => d)
            .on('click', d => {
                if (this.allConditionsTrue()) {
                ...
                const rowList: Map<string, boolean>[] = Array(
                    ...this.tempConditionFilterState.values()
                );
        ...
        this.yAxisFacetSelect
            ...
            .attr(
                'style',
                `max-width: ${maxLabelWidth}px; min-width: ${maxLabelWidth}px; height: ${maxLabelHeight}px`
            )
            ...
            .on('mouseenter', () =>
                this.miniCellSelect.selectAll('rect').classed('hovered', true)
            )
            .on('mouseleave', () =>
                this.miniCellSelect.selectAll('rect').classed('hovered', false)
            );
    ...
    private getGrowthLine(
        ...
        let avergeGrowthLine = [
            ...data.getAverageCurve(this.yKey, selection, this.smoothCurves)
        ];
    ...
    private getExtentOfGrowthCurves(
        facets: Map<string, Map<string, CurveList>>,
        ...
        for (let map of facets.values()) {
            for (let data of map.values()) {
                ...
                if (filteredOnly) {
                    ...
                    const allDataPoints = data.getAverageCurve(
                        this.yKey,
                        false,
                        this.smoothCurves
                    );
                ...
                let dataPoints = data.getAverageCurve(
                    this.yKey,
                    true,
                    this.smoothCurves
                );
                const shouldCheckFiltered =
                    (filteredOnly || this.data.brushApplied) &&
                    dataPoints.length > 0;
                if (shouldCheckFiltered) {
                    thisMin = Math.min(
                        thisMin,
                        d3.min(dataPoints, d => d[1])
                    );
                }
    ...
    private addApplyButton(container: HtmlSelection): void {
        let buttonSelect = container
            ...
            .on('click', () => {
                ...
                DevlibTSUtil.hide(
                    document.getElementById('conditionFilterApplyButton')
                );
                document.dispatchEvent(
                    new CustomEvent(DataEvents.applyNewFilter)
                );
            });
        DevlibTSUtil.hide(
            document.getElementById('conditionFilterApplyButton')
    ...
    private resetTempConditionFilter(): void {
        for (let [
            key,
            value
        ] of this.dataSuperset.conditionFilterState.entries()) {
    ...
    private tempConditionsDifferent(): boolean {
        for (let key1 of this.tempConditionFilterState.keys()) {
            let innerKeyVals = this.tempConditionFilterState
                .get(key1)
                .entries();
            for (let [key2, val] of innerKeyVals) {
                if (
                    val !==
                    this.fullData.conditionFilterState.get(key1)?.get(key2)
                ) {
                    return true;
                }
            }
    ...
    private drawLabels(labelData: [string, [number, number] | null][]): void {
        const indexedPoints: [string, [number, number] | null, number][] =
            labelData.map((d, i) => [d[0], d[1], i]);
        const validPoints = indexedPoints.filter((x, i) => x[1] !== null);
        const pixelSpacePoints: [number, number, number][] = validPoints.map(
            d => [this.scaleX(d[1][0]), this.scaleY(d[1][1]), d[2]]
        );
        this.averageCurveLabelContainer
            .selectAll('circle')
            ...
            .attr('r', 3)
            .attr('fill', d =>
                GroupByWidget.getColor(
                    labelData[d[2]][0].split('___'),
                    this.colorLookup
                )
            );
        ...
        const forceNodes: any[] = pixelSpacePoints.map(d => {
            return { x: d[0], y: d[1] };
        });
        this._forceSimulation = d3
        ...
        this.averageCurveLabelContainer
            ...
            .attr('stroke', (d, i) =>
                GroupByWidget.getColor(
                    labelData[pixelSpacePoints[i][2]][0].split('___'),
                    this.colorLookup
                )
            )
            .attr('fill', (d, i) =>
    ...
    private drawAverageLegend(): void {
        ...
        this.averageLegendSelect
            ...
            .attr(
                'x2',
                textOffset + selectedTextWidth + betweenPad + 2 * lineWidth
            )
            .attr('y1', 0)
            .attr('y2', 0)
            .attr('stroke-width', 1)
    ...
    private showLabel(): void {
        this.xLabelTextSelect.text(this.xKey).classed('noDisp', false);

        this.yLabelTextSelect.text(this.yKey).classed('noDisp', false);
    }
    ...
    protected OnResize(): void {
        if (this.data) {
            ...
            this.averageLegendSelect.attr(
                'transform',
                `translate(${this.margin.left / 2}, ${
                    this.margin.top + this.vizHeight + 44
                })`
            );
            this.facetLegendSelect.attr(
                'transform',
                `translate(${this.margin.left / 2}, ${
                    this.margin.top + this.vizHeight + 44
                })`
    ...
    private resizeBrush(): void {
        this.initBrush();
        if (
            this.lastYValueBrushBound == null ||
            this.lastXValueBrushBound == null
        ) {
            return;
        }
        ...
        this.brushGroupSelect.call(this.brush.move, [
            [left, top],
            [right, bottom]
        ]);
    }
