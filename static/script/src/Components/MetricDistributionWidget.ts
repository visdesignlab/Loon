...
import {
    Frame,
    MetricDistributionSubComponentTypes,
    Direction,
    MetricDistributionCollectionLevel,
    DatasetSpec
} from '../types';
...
export class MetricDistributionWidget extends BaseWidget<
    CurveList,
    DatasetSpec
> {
    constructor(
    ...
    protected Clone(
        ...
        return new MetricDistributionWidget(
            container,
            this.metricDistributionCollectionLevel,
            true
        );
    }
    ...
    private _subComponentLookup: Map<
        HTMLElement,
        MetricDistributionSubComponentTypes
    >;
    public get subComponentLookup(): Map<
        HTMLElement,
        MetricDistributionSubComponentTypes
    > {
    ...
    protected init(): void {
        ...
        this._layoutFramework = new LayoutFramework(
        ...
        this._subComponentLookup =
            this.layoutFramework.InitializeLayout<MetricDistributionSubComponentTypes>(
                layout
            );
        ...
        for (let [container, subComponent] of this.subComponentLookup) {
            switch (subComponent) {
                case MetricDistributionSubComponentTypes.Title:
                    this._titleContainerSelection = this.initSubComponent(
                        container,
                        'titleContainer'
                    );
                    let title: string;
                ...
                case MetricDistributionSubComponentTypes.BasisSelect:
                    this._basisSelectContainerSelection = this.initSubComponent(
                        container,
                ...
                case MetricDistributionSubComponentTypes.ScatterplotSelect:
                    ...
                    this._yAxisMatrixSelect = this.initSubComponent(
                        wrapper.node(),
                        'yAxisMatrixContainer'
                    );

                    let rightWrapper = wrapper
                        .append('div')
                        .classed('matrixRightWrapperContainer', true);

                    this._scatterPlotSelectContainerSelection =
                        this.initSubComponent(
                            rightWrapper.node(),
                            'matrixContainer'
                        );
                    this._xAxisMatrixSelect = this.initSubComponent(
                        rightWrapper.node(),
                        'xAxisMatrixContainer'
                    );
                ...
                case MetricDistributionSubComponentTypes.DistributionPlot:
                    this._distributionPlotContainerSelection =
                        this.initSubComponent(
                            container,
                            'distributionPlotContainer'
                        );
                    this.distributionPlotContainerSelection.node().style.minWidth =
                        '300px'; // this is hardcoded based on max width of histogram
                    this.distributionPlotContainerSelection.node().style.maxWidth =
                        '300px'; // this is hardcoded based on max width of histogram
                    break;
                case MetricDistributionSubComponentTypes.Scatterplot:
                    this._scatterPlotContainerSelection = this.initSubComponent(
                        container,
                        'scatterPlotOuterContainer'
                    );

                    this._scatterPlotContainerSelection.node().style.minWidth =
                        '300px'; // this is hardcoded based on max width of scatterplot
                    this._scatterPlotContainerSelection.node().style.maxWidth =
                        '300px'; // this is hardcoded based on max width of scatterplot
                    break;
                default:
                    break;
    ...
    public OnDataChange(): void {
        switch (this.metricDistributionCollectionLevel) {
            ...
            case MetricDistributionCollectionLevel.Curve:
                this._pointCollection = this.data
                    .curveCollection as PointCollection;
        ...
        for (let [
            index,
            attr
        ] of this.pointCollection.attributeList.entries()) {
    ...
    private updateUIData(): void {
        ...
        // const defaultIncluded = new Set(['Mass (pg)', 'Time (h)', 'Mass_norm', 'Track Length', 'Mass (pg) (avg)', 'Growth Rate', 'Exponential Growth Constant']);
        ...
        for (let [
            ...
            this.basisSelectionBooleans.push(
                rowIndex < maxDefaultMatrixSize && defaultIncluded.has(attr1)
            );
    ...
    private drawMatrixAxis(): void {
        ...
        this.yAxisMatrixSelect
            ...
            .on('click', d => {
                ...
                let row: boolWithIndex[] =
                    this.scatterplotSelectionBooleans[rowIndex];
                let allTrue = true;
                for (let cell of row) {
        ...
        const triangleButtonSvg = this.yAxisMatrixSelect
            .selectAll('svg')
            .data([42])
            .join('svg')
        ...
        const trianglePad = 4;
        const triangleScale = d3
            .scaleLinear()
            .domain([0, 1])
            .range([trianglePad, buttonWidth - trianglePad]);

        const gap = 0.05;
        const upperTrianglePoints: [number, number][] = [
            [gap, 0],
            [1, 0],
            [1, 1 - gap],
            [gap, 0]
        ];
        const lowerTrianglePoints: [number, number][] = [
            [0, gap],
            [1 - gap, 1],
            [0, 1],
            [0, gap]
        ...
        const xOffset =
            -0.5 *
            (buttonWidth +
                buttonWidth * Math.cos(-theta) +
                buttonHeight * Math.sin(-theta));
        const yOffset =
            0.5 *
            (buttonWidth * Math.sin(-theta) +
                buttonHeight * Math.cos(-theta) -
                buttonHeight);
        ...
        this.xAxisMatrixSelect
            ...
            .attr(
				...
				height: ${buttonHeight}px;
				transform: translate( ${
                    stepSize * (i + 1) + xOffset
                }px, ${yOffset}px) rotate(${rotate}deg);`
            )
            .text(d => d)
            .attr('title', d => d)
            .on('click', d => {
                ...
                for (let row of this.scatterplotSelectionBooleans) {
                    for (let cell of row) {
                        ...
                        if (
                            ...
                            this.basisSelectionBooleans[cellRowIndex]
                        ) {
                            if (!cell.value) {
                                allTrue = false;
                            }
    ...
    private getCurrentOptions(): string[] {
        return this.pointCollection.attributeList.filter(
            (d, i) => this.basisSelectionBooleans[i]
        );
    }
    ...
    private drawScatterPlotSelectContainerSelection(): void {
        ...
        this.scatterPlotSelectContainerSelection
            ...
            .attr(
                'id',
                d =>
                    'MetricDistributionWidget-scatterSelect-' +
                    d.index[0] +
                    '-' +
                    d.index[1]
            )
            .on('click', function (d, i) {
                ...
                let turnOn = !d.value;
                buttonSelect.classed('on', turnOn);
                thisWidget.scatterplotSelectionBooleans[d.index[0]][i].value =
                    turnOn;
                if (i === d.index[0]) {
                    thisWidget.updateHistograms();
                } else {
            ...
            .on('mouseenter', function (d) {
                ...
                thisWidget.yAxisMatrixSelect
                    .selectAll('button')
                    .data(options)
                    .classed('hovered', d => d === rowName);

                thisWidget.xAxisMatrixSelect
                    .selectAll('button')
                    .data(options)
    ...
    private drawHistograms(): void {
        ...
        let parentElement =
            this.distributionPlotContainerSelection.node().parentElement;
        ...
        this.distributionPlotContainerSelection
            ...
            .each(function (d) {
                ...
                const canBrush = true;
                let newWidget = new HistogramWidget(
                    container,
                    d,
                    canBrush,
                    thisWidget.includeExemplarTrackButton
                );
    ...
    private drawScatterPlots(flatData: boolWithIndex[]): void {
        ...
        let parentElement =
            this.scatterPlotContainerSelection.node().parentElement;
        parentElement.classList.remove('noDisp');
    ...
    private shouldHide(d: boolWithIndex | number): boolean {
        ...
        if (
            !this.basisSelectionBooleans[d.index[0]] ||
            !this.basisSelectionBooleans[d.index[1]]
        ) {
            return true;
        }
        return !d.value;
    ...
    private resizeSubComponents(): void {
        ...
        this.scatterPlotContainerSelection.node().style.maxHeight =
            this.height + 'px';
    }
}
