...
import {
    ImageLabels,
    ImageStackDataRequest,
    Row
} from '../DataModel/ImageStackDataRequest';
...
export class ImageStackWidget {
    constructor(
        ...
        this._imageTrackWidget = new ImageTrackWidget(
            imageTrackContainer,
            this,
            samplingStratOptions
        );
    ...
    public get numPixelsInTile(): number {
        return (
            this.imageStackDataRequest?.tileWidth *
            this.imageStackDataRequest?.tileHeight
        );
    }
    ...
    public init(): void {
        ...
        this._invertImageToggle = this.toggleOptionsContainer
            .append('input')
            .attr('type', 'checkbox')
            .on('change', () => {
        ...
        this._imageAndLegendContainer = this.innerContainer
            .append('div')
            .classed('imageAndLegendContainer', true);

        this._selectedImageContainer = this.imageAndLegendContainer
            .append('div')
        ...
        this._legendToggleContainer = this.imageAndLegendContainer
            .append('div')
            .classed('toggleOptions', true)
        ...
        this._legendToggleSelected = this.legendToggleContainer
            .append('input')
            .attr('type', 'checkbox')
            .on('change', () => {
                this.OnBrushChange();
        ...
        this._legendToggleFilteredOut = this.legendToggleContainer
            .append('input')
            .attr('type', 'checkbox')
            .on('change', () => {
                this.OnBrushChange();
        ...
        this.legendToggleContainer
            ...
            .classed('green', true)
            .text('Filtered Out');
        (this.legendToggleFilteredOut.node() as HTMLInputElement).checked =
            true;

        this._legendToggleNotSelected = this.legendToggleContainer
            .append('input')
            .attr('type', 'checkbox')
            .on('change', () => {
                this.OnBrushChange();
        ...
        this.legendToggleContainer
            ...
            .classed('blue', true)
            .text('Not Selected');
        (this.legendToggleNotSelected.node() as HTMLInputElement).checked =
            true;
        ...
        document.addEventListener('launchExemplarCurve', (e: CustomEvent) => {
            ...
            this.setNumExemplars();
            let buttonChangeEvent = new CustomEvent('changeModeSelect', {
                detail: 0
            });
        ...
        document.addEventListener('modeChange', (e: CustomEvent) => {
            ...
            document.dispatchEvent(
                new CustomEvent('exemplarAttributeChange', {
                    detail: this.inExemplarMode ? this.exemplarAttribute : null
                })
            );
        });
    ...
    private updateCanvas(skipImageTrackDraw = false): void {
        ...
        if ((this.showOutlineToggle.node() as HTMLInputElement).checked) {
            this.imageStackDataRequest?.getLabel(
    ...
    private updateTracksCanvas(): void {
        ...
        if (this.inExemplarMode) {
            ...
            for (let curveList of [justData, this.manuallyPinnedTracks]) {
                for (let curve of curveList) {
                    ...
                    for (let i = 0; i < this.condensedModeCount; i++) {
                        const point: PointND =
                            this.imageTrackWidget.getPointInCondensedMode(
                                curve,
                                i
                            );
            ...
            if (!this.exemplarLocations.has(this.getCurrentLocationId())) {
                eventToDispatch = new CustomEvent('locFrameClicked', {
                    detail: {
    ...
    private getExemplarCurves(): conditionExemplar<CurveND>[] {
        ...
        for (let facet of this.facetList) {
            ...
            const samplingStat =
                this.imageTrackWidget.currentSamplingStategy.strat;
            ...
            for (let p of percentages) {
                ...
                let exemplarCurve: CurveND = quickSelect(
                    tracks,
                    index,
                    (curve: CurveND) => curve.get(this.exemplarAttribute)
                );
                conditionList.push({ data: exemplarCurve, type: 'auto' });
            }
            for (let val of this.imageTrackWidget.manualSampleValues) {
                let closestCurve = this.getClosestCurve(
                    tracks,
                    val,
                    this.exemplarAttribute
                );
                conditionList.push({
                    data: closestCurve,
                    type: 'manual',
            ...
            conditionList.sort(
                (a, b) =>
                    a.data.get(this.exemplarAttribute) -
                    b.data.get(this.exemplarAttribute)
            );
    ...
    private getCurvesBasedOnPointsAtCurrentFrame(): conditionExemplar<CurveND>[] {
        let curveList: conditionExemplar<CurveND>[] = [];
        const pointsAtFrame = this.data.GetCellsAtFrame(
            this.getCurrentLocationId(),
    ...
    private createOutlineImage(
        ...
        let myImageData = this.canvasContext.createImageData(
            this.imageStackDataRequest.tileWidth,
            this.imageStackDataRequest.tileHeight
        );

        for (
            let rowIdx = firstIndex;
            rowIdx < firstIndex + this.imageStackDataRequest.tileHeight;
            rowIdx++
        ) {
            let row: Row = rowArray.rowList[rowIdx];
            for (let labelRun of row.row) {
                for (
                    let colIdx = labelRun.start;
                    colIdx < labelRun.start + labelRun.length;
                    colIdx++
                ) {
                    if (
                        this.isBorder(labelRun.label, rowIdx, colIdx, rowArray)
                    ) {
                        let flatIdx =
                            (rowIdx - firstIndex) *
                                this.imageStackDataRequest.tileWidth +
                            colIdx;
                        flatIdx *= 4;
                        let [cell, _index] = this.getCell(
                            labelRun.label,
                            this.data
                        );
                        let { color: color, show: show } =
    ...
    private clearCanvas(): void {
        this.canvasContext.clearRect(
            0,
            0,
            this.imageStackWidth,
            this.imageStackHeight
        );
        this._defaultCanvasState = this.canvasContext.createImageData(
            this.imageStackDataRequest.tileWidth,
            this.imageStackDataRequest.tileHeight
        );
    }
    ...
    private updateBasedOnMousePosition(forceDraw: boolean = false): void {
        ...
        this.imageStackDataRequest.getLabel(
            ...
            (rowArray: ImageLabels, firstIndex: number) => {
                ...
                const label = ImageStackDataRequest.getLabelValue(
                    rowIdx,
                    colIdx,
                    rowArray
                );
    ...
    public togglePin(track: CurveND): void {
        ...
        this.updateCanvas();
        const manualPinToggleEvent = new CustomEvent('manualPinToggle', {
            detail: this.manuallyPinnedTracks
        });
    ...
    public showSegmentHover(
        rowArray: ImageLabels,
        segmentId: number,
        firstIndex: number,
        showTooltipImmediately: boolean = false
    ): void {
        ...
        let pageY = 0;
        if (cell) {
            let canvasBoundRect = this.selectedImageCanvas
                .node()
                .getBoundingClientRect();
            cellX =
                (cell.get('X') + cell.parent.get('xShift')) /
                this.imageStackDataRequest.scaleFactor;
            cellY =
                (cell.get('Y') + cell.parent.get('yShift')) /
                this.imageStackDataRequest.scaleFactor;
        ...
        let tooltipContent: string = this.getTooltipContent(
            segmentId,
            cell,
            index
        );
    ...
    public getLabelIndexFromBigImgPixelXY(
        frameIndex: number,
        x: number,
        y: number
    ): [number, number] {
        ...
        let rowIdx =
            (y % this.imageStackDataRequest?.tileHeight) +
            (frameIndex % this.imageStackDataRequest?.tilesPerFile) *
                this.imageStackDataRequest?.tileHeight;
    ...
    public getTileIndexFromBigImgPixelXY(x: number, y: number): number {
        ...
        return (
            rowIndex * this.imageStackDataRequest?.numberOfColumns + colIndex
        );
    }
    ...
    public getCell(
        label: number,
        dataSource: CurveList
    ): [PointND, number] | [null, null] {
        return dataSource.GetCellFromLabel(
            this.getCurrentLocationId(),
            this.getCurrentFrameId(),
            label
        );
    ...
    public getCellColor(cell: PointND | null): {
        ...
        if (!cell) {
            ...
            color = [119, 140, 77];
            show = (this.legendToggleFilteredOut.node() as HTMLInputElement)
                .checked;
        } else if (cell.inBrush) {
            // FireBrick
            color = [178, 34, 34];
            show = (this.legendToggleSelected.node() as HTMLInputElement)
                .checked;
        } else {
            // SteelBlue
            color = [70, 130, 180];
            show = (this.legendToggleNotSelected.node() as HTMLInputElement)
                .checked;
        }
