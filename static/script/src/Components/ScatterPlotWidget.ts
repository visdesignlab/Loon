...
> {
    ...
    protected Clone(
        container: HTMLElement
    ): BaseWidget<PointCollection, DatasetSpec> {
    ...
    private drawPoints(): void {
        let validPoints = this.data.Array.filter((point: NDim) => {
            return !isNaN(point.get(this.xKey)) && !isNaN(point.get(this.yKey));
        });
    ...
    private drawAxis(): void {
        this.xAxisGroupSelect.call(d3.axisBottom(this.scaleX).ticks(5));

        this.yAxisGroupSelect.call(d3.axisLeft(this.scaleY).ticks(5));
    }
